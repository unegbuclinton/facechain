...
const useFetchPredictions = () => {
    ...
    const fetchPredictions = async () => {
        ...
        try {
            ...
            const predictions = await res.map((prediction)=> new Predictions().fromJson(prediction))
            setIsLoading(false)
            dispatch({type: "GET_ALL_PREDICTIONS", payload: predictions.sort((a, b) => b.id - a.id)})
        } catch (error) {
