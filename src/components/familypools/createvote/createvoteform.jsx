...
const CreateVoteForm = () => {
    ...
    const tabTitles = ['Select the topic', 'Select the pair', 'Enter a quantity and price', 'Enter a Deadline', 'Propose a sale'];
    ...
    return (
        <div className="bg-darkGrey flex flex-col relative min-h-[500px] rounded-lg">o
           <div className="py-10  text-white">
              <div className="flex justify-between px-4 sm:px-10 mb-8">
                  <button onClick={handleCancel}>Cancel</button>
                  <p>{tabTitles[activeTab]}</p>
                  <button className="text-primary" onClick={handleNext}>{topic == "Sell" ? activeTab === 4 ? "Create Vote"
                : "Next" : activeTab === 3 ? "Create Vote" : "Next"}</button>
              </div>
              {activeTab === 0 && <ChooseTopic action={handleChooseTopic}/>}
              {activeTab === 1 && <ChoosePair selectedPairs={selectedPairs} action={handlePairSelect}/>}
              {activeTab === 2 && <SetQuantityPrice selectedPairs={selectedPairs}/>}
              {activeTab === 3 && <ChooseDeadline start={startPeriod} end={endPeriod} action={handleStartPeriod} actionTwo={handleEndPeriod}/>}
              {activeTab === 4 && <ProposeSale selectedPairs={selectedPairs}/>}
           </div>
        </div>
    );
}
