...
const PredictionList = ({data}) => {
    ...
    const [isMobile, setIsMobile] = useState(window.innerWidth >= 320 && window.innerWidth <= 768);
    ...
    return (
        ...
        <div className='predictions-list w-full flex flex-wrap justify-between'>
            {predictions.map((prediction, index) => (
                 <div className='bg-darkGrey px-3 py-7 rounded-lg text-white w-full md:w-[48%] mb-11' key={index}>
                 <div className='flex justify-between'>
                     ...
                     <div className='text-right w-[30%] flex justify-end'>
                        {!isMobile && <More user_id={prediction.user_id}  predictionId={index} openModal={openDeleteModal} />}
                        {isMobile && <MoreMobile user_id={prediction.user_id} toggleMoreMobile={toggleMoreMobile} predictionId={index} openModal={openDeleteModal}/>}
        ...
        <DeletePrediction ref={deleteRef} predictionId={predictionId} />
        {/* empty space :-) */}
