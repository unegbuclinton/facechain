...
const AddPredictionForm = () => {
    ...
    const [endPeriod, setEndPeriod] = useState(startPeriod ? new Date(startPeriod).fp_incr(1) : new Date());
    const [startPrice, setStartPrice] = useState(35000)
    const [hasTab, setHasTab] = useState(null)
    const [endPrice, setEndPrice] = useState('')
    const navigate = useNavigate()
    const {isLoading, error, isUploaded, setIsUploaded, createPrediction }  = useCreatePrediction()

    const {diffInDays, diffInHours, diffInMinutes, days} = getNumberOfDays(endPeriod)

    //expected Rate Change
    let startprice = parseFloat(startPrice)
    let expectedprice = parseFloat(endPrice)
    let target_1_move = (((expectedprice-startprice)/startprice)*100).toFixed(2)
    ...
    return (
        <div className="flex flex-col relative">
           <div className="bg-darkGrey px-4 sm:px-10 py-10 rounded-lg addpredictionform">
                ...
                {activeTab === 2 && <ChoosePeriod startPeriod={startPeriod} endPeriod={endPeriod} action={handleStartPeriod} actionTwo={handleEndPeriod}/>}
                {activeTab === 3 && <ChooseExpectedPrice startPrice={startPrice} endPrice={endPrice} action={handlePriceInput} />}
                {activeTab === 4 && <CheckPrediction startPrice={startPrice} endPrice={endPrice} startPeriod={startPeriod} endPeriod={endPeriod} pair={currencyPair} target_1_move={target_1_move}/>}
                <div className="flex justify-center absolute top-4 right-0 left-0 md:relative">
                    <ProgressDots data={data} activeTab={activeTab} />
                </div>
           </div> 
           <button className="bg-primary self-center text-xl font-bold rounded-full px-32 py-4 m-10" onClick={validateAndNext}>
              {isLoading ? (
							<FontAwesomeIcon
							icon={faSpinner}
